...
var dataFactory = function(identifier, dataName) {
    switch(dataName)  {
        ...
        case 'catData':
            return Promise.all([waterline.catalogs.findLatestCatalogOfSource(identifier,'ohai'), waterline.catalogs.findLatestCatalogOfSource(identifier,'dmi')])
            ...
            entry.discoveredDeviceInfoList.forEach(function(device){
                ...
                .then(function() {
                    if (self.redfishUDM){
                        ...
                        console.log("GPVP: newNode.name = %s, redfishUDM = %j", newNode.name, self.redfishUDM);
			var hardware = dataFactory(newNode.name, 'hardware');
                        var chassis = dataFactory(newNode.name, 'chassis');
                        console.log("catalogs %j, %j",hardware,chassis);
                    }
                })
                .catch(function(err){
                    logger.error(err.errorMsg);
